(ns ov-movies.crawl.crawler
  (:require [next.jdbc :as jdbc]
            ...
            [ov-movies.screening :refer
             [insert-screenings! remove-screenings-for-movies!]]
  ...
  (let [meta-data (movie-api/search-movie api-key (:title movie))
    ...
    (merge movie
           {:id id,
            ...
            :poster (str "https://image.tmdb.org/t/p/w500"
                         (:poster-path meta-data)),
            :description (:overview meta-data),
            :screenings screenings})))

(def cinemas
  [[:cineplex-germering cineplex-germering/scrape!]
...
(defn- add-cinema
  ...
  (update movie
          :screenings
          (fn [screenings] (map #(assoc % :cinema (name cinema)) screenings))))

(defn- merge-by
  [compare-fn merge-fn coll]
  (let [groups (group-by compare-fn coll)]
    (map (fn [[_k vals]] (merge-fn vals)) groups)))

(defn- merge-same-movies
  [movies]
  (merge-by :id
            (partial reduce
                     (fn [acc movie]
...
(defn crawl!
  ...
  (let [movies (->> cinemas
                    (mapcat (fn [[cinema scrape!]]
                              (map #(add-cinema % cinema) (scrape!))))
                    (pmap #(add-movie-metadata movie-db-api-key %))
                    (merge-same-movies)
                    ;; Movies that are not in "The Movie Database" will be
                    ;; dropped here
                    (remove #(empty? (:id %))))
        screenings (mapcat :screenings movies)]
    (jdbc/with-transaction [tx db]
                           (insert-movies! tx
                                           (map #(dissoc % :screenings) movies))
