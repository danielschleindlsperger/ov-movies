...
;;;
;;; Detail page: Load and parse a detail page and retrieve all screening of the
;;; movie.

(defn- parse-movie-id
  "Takes a web page as a hickory data structure and finds the cineplex id."
  [hick]
  (let [meta-tag (sel/select (sel/child (sel/tag :head)
                                        (sel/and (sel/tag :meta)
                                                 (sel/attr
                                                   :name
                                                   #(= % "cineplex:filmId"))))
                             hick)
...
(defn- parse-screening
  ...
  (let [time-el (first (sel/select (sel/tag :time) show))
        ...
        time (-> time-el
                 ...
                 str/trim
                 (str/replace #":" "-"))]
    {:date (parse-date (str date "-" time)), :original? (original? show)}))
...
(defn- parse-screenings
  "Takes a web page as a hickory data structure and parses the movie's screening dates.
   ...
   :original"
  [hick]
  (let [screenings-elements (sel/select (sel/child
                                          (sel/class "performance-holder")
                                          (sel/class "schedule__grid-item"))
                                        hick)]
...
(defn scrape!
  ...
  (let [overview-html (slurp overview-url)
        movies (pmap (comp parse-movie fetch-detail-page)
                     (detail-urls overview-html))]
    movies))
