...
(defn- combine-screenings
  [screenings]
  (reduce (fn [movie screening]
            (merge movie
                   {:id (or (:movie-id movie) (:movie-id screening)),
                    :title (or (:movie-title movie) (:movie-title screening)),
                    :screenings (conj (:screenings movie)
                                      {:id (:id screening),
                                       :date (:date screening),
...
(defn- extract-movies-with-screenings
  [events]
  (->> events
       (map (fn [screening]
              {:movie-id (get-in screening [:workPresented (keyword "@id")]),
               ...
               :original? (original? screening)}))
       (group-by :movie-id)
       (map (fn [[_ screenings-for-movie]]
              (combine-screenings screenings-for-movie)))))

(defn parse-movies
  [overview-html]
  (let [overview-htree (-> overview-html
                           parse
                           as-hickory)
        ld-json (first (sel/select
                         (sel/and (sel/tag :script)
                                  (sel/attr :type #(= % "application/ld+json")))
                         overview-htree))
        screening-events (-> ld-json
                             :content
                             first
                             (json/read-str :key-fn keyword))]
    (extract-movies-with-screenings screening-events)))

(defn scrape! [] (parse-movies (slurp overview-url)))
