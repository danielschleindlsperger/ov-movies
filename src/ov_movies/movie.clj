...
(defn get-movies-with-upcoming-screenings [db]
                                     (map parse-pg-movie (jdbc/query db [get-movies-query])))
(defn insert-movies! [db movies]
                (jdbc/query db (insert-movies-query movies)))
(defn blacklist-movie! [db {id :id}]
                  (jdbc/query db (blacklist-movie-query id)))