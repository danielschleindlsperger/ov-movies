...
(s/def ::id
  (s/with-gen (s/and string? #(= 6 (count %)))
...
(s/def ::movie (s/keys :req [::id ::title] :opt [::poster]))

(defn insert-movies-query
  [movies]
...
(sql-format {:select [:*],
             :from [:screenings],
             :where [:and [:in :movie_id (map :movie-id [{:movie-id "foo"}])]
                     [:= :blacklisted false]]})

(defn get-movies-with-upcoming-screenings
  [db]
  (jdbc/with-transaction
    [tx db]
    (let [upcoming-screenings (jdbc/execute!
                                tx
                                (sql-format {:select [:*],
                                             :from [:screenings],
                                             :order-by [[:date :asc]],
      ...
      (map (fn [movie]
             (assoc movie
               :screenings (get screenings-by-movie-id (:id movie) [])))
        upcoming-movies))))

(defn insert-movies!
  [db movies]
  (let [stmt (insert-movies-query movies)] (jdbc/execute! db stmt db-opts)))

(defn blacklist-movie!
  [db {id :id}]
  (jdbc/execute! db (blacklist-movie-query id) db-opts))