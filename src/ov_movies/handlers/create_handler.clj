...
(defn wrap-movie-db
  ...
  (let [api-key (-> config
                    :movie-db
                    :api-key)]
    (fn [req] (handler (assoc req :movie-db-api-key api-key)))))

(defn wrap-passphrase
  [handler passphrase]
  (fn [req] (handler (assoc req :passphrase passphrase))))

(defn decode-base64 [^String s] (String. (.decode (Base64/getDecoder) s)))
      ...
      (if (= supplied-password password)
        ...
        {:status 401,
         :headers {"WWW-Authenticate"
                     "Basic realm=\"Allow triggering a crawl\"",
                   "content-type" "text/basic"},
         :body
           "Please enter the correct password. You can omit the username."}))))
